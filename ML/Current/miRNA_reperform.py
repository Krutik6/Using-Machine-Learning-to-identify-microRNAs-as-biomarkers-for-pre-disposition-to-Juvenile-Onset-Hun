...
Data = os.path.join(DATA_DIR, "10m_miRNA_r.csv")
ValData = os.path.join(DATA_DIR, "2m_miRNA_r.csv")
...
# f10 = kbest.findkbest(df10train, df10test, feats=26)
...
# f2 = kbest.findkbest(df2train, df2test, feats=14)
# k2train, k2test = kbest.ktodf(f2, df2train, df2test)
# ada.gridne(k2train, k2test)
# ada.gridlr(k2train, k2test, ne=50)
# ada.predict(k2train, k2test, lr=1, ne=50)

# fage = kbest.findkbest(df10, df2, feats=53)
# kagetrain, kagetest = kbest.ktodf(fage, df10, df2)
# ada.gridne(kagetrain, kagetest) 
# ada.gridlr(kagetrain, kagetest, ne=10)
# ada.predict(kagetrain, kagetest, lr=0.01, ne=10)
###############################################################################
...
# extra.predict(k10train, k10test, ne=500, ms=3)

# f2 = kbest.findkbest(df2train, df2test, feats=14)
# k2train, k2test = kbest.ktodf(f2, df2train, df2test)
# extra.numtrees(k2train, k2test) 
# extra.numsplits(k2train, k2test, ne=1000)
# extra.predict(k2train, k2test, ne=1000, ms=10)

# fage = kbest.findkbest(df10, df2, feats=53)
# kagetrain, kagetest = kbest.ktodf(fage, df10, df2)
# extra.numtrees(kagetrain, kagetest) 
# extra.numsplits(kagetrain, kagetest, ne=100) 
# extra.predict(kagetrain, kagetest, ne=100, ms=5)
###############################################################################
#For RandomForest get best params and training and testing results
# f10 = kbest.findkbest(df10train, df10test, feats=26)
# k10train, k10test = kbest.ktodf(f10, df10train, df10test)
# rf.paramfind(k10train, k10test)
# rf.predict(k10train, k10test, ne=500, ml=1, ms=2, md=5)

# f2 = kbest.findkbest(df2train, df2test, feats=14)
# k2train, k2test = kbest.ktodf(f2, df2train, df2test)
# rf.paramfind(k2train, k2test)
#rf.predict(k2train, k2test, ne=10, ml=1, ms=2, md=5)

# fage = kbest.findkbest(df10, df2, feats=53)
# kagetrain, kagetest = kbest.ktodf(fage, df10, df2)
# rf.paramfind(kagetrain, kagetest)
# rf.predict(kagetrain, kagetest, ne=50, ml=1, ms=2, md=5)
###############################################################################
#best cases
#10m
